<template>
  <div v-if="account && Object.keys(account).length > 0" class="flex justify-center items-center text-sm">
    <slot></slot>
    <button v-if="needEdit" type="button" class="flex items-center mr-3 focus:outline-none"
    ...
    <button v-if="needDel" type="button" class="flex items-center text-red-600 focus:outline-none"
