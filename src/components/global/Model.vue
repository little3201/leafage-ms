<template>
  <transition name="model">
    ...
    >
      <div
        class="flex items-end justify-center min-h-screen pt-4 px-4 pb-10 text-center sm:block sm:p-0"
      >
        <span class="inline-block align-top sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
        <div
        ...
        >
          <div class="bg-white p-4 sm:p-6 max-h-96 overflow-auto sm:max-h-full">
            <slot></slot>
          </div>
          <!--footer-->
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse text-sm">
            ...
            <button
              ...
              class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto active:cursor-wait"
            >取消</button>
          </div>
        </div>
      </div>
